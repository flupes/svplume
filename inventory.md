---
title: Inventory
permalink: /inventory/
layout: default
---

# 2002 Nor’Sea 27 \- Inventory

{% picture listing/banner-left_DSCF6903.jpg --alt Plume 2025
  --link https://photos.flupes.org/Public/Plume/NorSea27-Plume%}
<br>
*Recent key upgrades have dates noted in bold*

[Listing Photo Gallery](https://photos.flupes.org/Public/Plume/NorSea27-Plume) --- [Plume Listing]({%link listing.md%})
{: style="text-align: center; font-weight: bold;"}

## Sails & Rig

* Mainsail (dacron, 3-reef, **2022**)
* 80% jib (dacron, **2024**)
* Genoa (dacron, original serviced in **2022**)
* Storm jib (dacron, excellent condition)
* Spinnaker (excellent condition)
* Spinnaker pole
* Standing rigging (**2019**)
* Running rigging (gradually replaced between **2019 and 2024**)
* Aluminium mast and boom
* Chainplates: stainless steel
* Two new spare halyards with shackles and multiple spare lines

## Deck & Ground Tackle

* Winches: Lewmar Ocean Series (three \#30ST in cockpit, one \#16ST on mast)
* Schaefer organizers (2-block & 3-block)
* Fiberclass deck with molded non-skid
* Primary anchor: Spade A80 15 lb aluminum (**2020**)
  * 1/4” ACCO Grade 43 chain x 120 ft (**2020**)
  * 1/2” Samson Super Strong double-braid nylon line x 150 ft
  * Total 270 ft of rode
* Storm anchor: M2 Mantus 35 lbs (**2020**)
  * 5/16″ chain x 50 ft \+ 5/8″ 3-strand x 250ft
* Stern anchor: Fortress FX-23
  * 1/4″ chain x 25 ft \+ 1⅜″ webbing x 150 ft on roller
* Long floating line for mooring
* Sea anchor
* Various mooring/docking lines with 6 fenders

## Self-Steering & Autopilot

* Self-Steering: Monitor windvane (serviced in **2024**)
* Autopilot: Simrad Tillerpilot TP22

## Electronics & Navigation

* Nav station with 15″ display \+ low-power single board computer for OpenCPN cartography (**2019**)  
* AIS WatchMate XB-8000 transceiver with masthead antenna  
* Vesper Marine VHF/AIS/FM antenna splitter VM-SP160   
* VHF: Standard Horizon HX870 handheld  
* SSB: Icom IC-706MK2G \+ AH-4 automatic tuner  
* GPS/Chartplotter: Garmin ECHOMAP Plus 74cv with BlueChart G3 Charts and Transducer, 7” touchscreen color display (**2019**)  
* Wind: Davis Windex masthead vane  
* Compass: Ritchie Powerdamp  
* Stereo: Kenwood KDC-MPV7019 AM/FM/CD with Bose speakers  
* USB chargers on the chart table and front of main cabin

## Electrical & Power

* Lifeline Starting (GPL-2400T) engine battery (**2021**)
  * 870 CCA
* (2) Lifeline Deep Cycle (GPL-24T) house battery (**2021**)
  * 160Ah in parallel
* Solar panels
  * Kyocera 50 W solar panel
  * 35 W mobile
  * Morningstar ProStar PS-15 solar controller
* Balmar 80A alternator w/ ARS-5 regulator \+ DuoCharge
* ProTech 1220i 20 A (3 banks) shore charger
* Xantrek LinkLite battery monitor (measure real charge)
* 3 circuit breakers for the 2 battery bank allows optimal switching in emergency

## Safety

* Liferaft: Viking RescYou 4-person liferaft (**2020**, service due)
* EPIRB ACR ResQFix PLB-300 Personal Locator Beacon
* Weems & Plath night visual distress signals
* (2) LifeSling Man Overboard Rescue System
* Forespar floating automatic strobe water flashing light
* (2) Type IV cushions
* (2) Life jackets
* Horn: Safety Blaster
* Jacklines
* Deck ladder
* Bilge pumps: Whale manual in cockpit, Rule 2000 automatic electric
* Fire protection
  * Kidde extinguisher Halotron 9lb, (2) Kidde extinguishers 4lb
  * Fire blanket (galley)
  * Fire port to engine

## Interior

* Main cabin saloon  
  * Cushions (**2020**)
  * U-shaped dinette 
  * Sliding centerline table (forming a large berth when lowered)
  * (2) Leecloths
  * Shelves and abundant stowage around/under settees
  * Large forward locker with chain-locker access
  * Chart table with navigation station
  * Weems & Plath clock and barometer
* Galley
  * Wallas diesel two-burner stove and cabin heater (serviced in **2020**)
  * Origo 1500 gimbaled alcohol one-burner stove
  * Single-basin stainless primary sink
  * Single-basin built-in secondary sink
  * Whale freshwater foot pump (**2019**) and Fynspray brass WS66 spout (**2019**)
  * Whale salt-water hand pump/spout
  * Large built-in icebox
  * Shelves, cutlery drawer, pantry locker, stowage
* Head
  * Air Head composting toilet
  * Shelves and lockers
  * Hooks for hanging wet gear
  * Full-length mirror
* Aft cabin
  * Double berth
  * Two large lockers (port & starboard)
  * Ample access to engine compartment
* Lighting
  * 3 Alpenglow with teak frame lights: 2 modified CF with two levels in the cabin and 1 white \+ red LED on the chart table
  * 3 reading lights 3 (LED and CF aft cabin) \+ 1 LED light in the head
* Ventilation
  * 1 dorade (main cabin)
  * 1 fan intake (aft cabin)
  * 2 adjustable, silent, teak frame fans in the main cabin
  * 1 fan in the aft cabin

## Engine & Mechanical

* Engine: Yanmar 2GM20F
* Seawater strainer: GROCO ARG-500-S bronze
* Rudder gudgeon bushings replaced in **2019**
* Prop: 2-blade Michigan bronze 13x12 (**2024**)

## Tanks & Plumbing

* Fuel: 1 × 30 gal aluminum tank (in bilge under engine, glassed in)
* Water: 2 × 15 gal plastic tanks (under aft cabin stowage, framed and secured)
* (5) Thru-hulls: Bronze
  * 2 below the waterline (replaced in **2019** and **2024**)
  * 3 above the waterline
* Speedometer / temperature sensor ST300
* Depth sounder Garmin GT15M-IH transducer mounted inside the hull (**2019**)
* PowerSurvivor 40E watermaker: all connections in place, but watermaker has been taken out of the boat and would need service

## Canvas & Covers

* Dodger (**2008**, serviced in **2019**)
* Companionway covers (main and aft cabins, **2019**)
* Forward hatch cover (**2019**)
* Mainsail cover
* Tiller cover
* Bowsprit cover
* Full set of toe rail covers
* (2) Headsail deck bags
* Portlight curtains
* Portlight mosquito nets
* Bimini with mosquito-net enclosure
* Water-bottles holder
* Sunglasses/phones holder
* Tool box

## Miscellaneous

* Teak cockpit grate
* Cockpit table (using large section of cockpit grate)
* Swim platform (using small section of cockpit grate)
* Various spare parts (engine, rigging, etc.)
* Bosun’s chair
* User manuals
* Plume is US documented

## Trailer

* PACIFIC G27TRI5-15 Tri-Axle Nor’Sea 27 Special Trailer
* Purchased new in **2009** for $8995
* Disc Brakes
* Special Roller System
* 3x5 Tube Frame
* 2500\# Drop Leg Jack
* LED Lighting
* Radial Tires and Galvanized Wheels
* Spare tire & wheel & carrier
* Stored on blocks with wheel covers, rollers stored inside, may require service


[Back to Plume Listing]({%link listing.md%})